...
//----------------------------------------------------------
//----------------------------------------------------------
int i2cread(int offset, char *buf, int len)
{
	*buf = (char)offset;
	if (i2cwrite(buf,1))
		myReturn(1);
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
BYTE nctrb_mc_io_readLineSensor(void){
...
void nctrb_mc_io_writePortOUT1500(WORD value){
  ...
  buf[00]	= 0x01;						//メカコンI2Cの書き込み先アドレス
  buf[01]	= (0xFF & (value >> 8));	//書き込むデータ 1byte目
  buf[02]	= (0xFF & (value >> 0));	//書き込むデータ 2byte目
  if (i2cwrite(buf, 3)){
	printf("Error: i2c write(). \n");
	exit(1);
  }
